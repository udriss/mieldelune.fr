...
export async function POST(req: Request) {
    try {
        const { showDescription } = await req.json();

        if (typeof showDescription !== 'boolean') {
            return NextResponse.json({ 
                success: false, 
                message: 'Invalid "showDescription" value' 
            }, { status: 400 });
        }
        ...
        siteData.showWeddingDescription = showDescription;
        await writeSiteData(siteData);

        return NextResponse.json({ 
            success: true, 
            message: 'All descriptions visibility updated successfully.' 
        });
    } catch (error) {
        console.error('Error in /api/updateAllDescriptions:', error);
        return NextResponse.json({ 
            success: false, 
